...
<%
    ...
    try
    {
        ...
        int i=st.executeUpdate("insert into doctor values('"+t1+"','"+t2+"','"+t3+"','"+t4+"','"+t5+"','"+t6+"','"+t7+"','"+t8+"')");
        if(i>0)
        {
            response.sendRedirect("Doctor.jsp?msg=Register Sucess");
        }
        else
        {
         response.sendRedirect("DoctorReg.jsp?msg=Register Fail");   
        }
    }
    catch(Exception e)
    {
